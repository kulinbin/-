...
original_image_file='guli.jpg'
style_image_file='star.jpg'
...
#使用scipy模块加载两幅图片，并且将风格图片的维度调整的和原始图片一致
...
#基于上述加载的权重和网络层定义，通过tensorflow的内建函数创建网络
...
#在本例子中，原始图片采用relu4_2层，风格图片采用reluX_1层组合
original_layer='relu4_2'
style_layers=['relu1_1','relu2_1','relu3_1','relu4_1','relu5_1']

#运行extract_net_info()函数获取网络权重和平均值，调整图片形状为四维，方便tensorflow的图像操作
...
#为创建综合图片，开始加入随机噪声，运行网络
initial=tf.random_normal(shape)*0.05
image=tf.Variable(initial)
vgg_net=vgg_network(network_weights,image)

#声明第一个损失函数，定义为原始图片relu4_2层输出与归一化的原始图片的输出差值的L2范数
...
#最小化总的损失函数，总的损失函数是原始图片损失，风格图片损失和总变分损失的组合
